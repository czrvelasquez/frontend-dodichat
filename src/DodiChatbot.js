...
`;
        try {
            ...
            const cleanedData = data2.data.replace(/```json|```/g, '');
            setHerramientasEvaluacionTexto(cleanedData);
            setIsPDFReady(true);
        } catch (error) {
    ...
    const handleRegistration = async () => {
        if (!registerName || !registerEmail || !registerConfirmEmail || !registerWhatsapp) {
    ...
    const sendMessage = async (selectedOption = null) => {
        ...
        if (step === 0) {
            ...
            if (emailRegex.test(inputToSend)) {
                ...
                await enviarDatosPlanAlServidor();
                setStep(13);
                if (isPremium) {
                    await descargarDOCX();
                } else {
